    ...
    <div class="card-body p-4">
      ...
      <form class="forms-sample" method="POST" action="{{ route('users.update', $user) }}">
        ...
        <div class="form-group row mb-3">
          ...
          <div class="col-sm-9">
            <input type="text" name="name" value="{{ $user->name }}" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Jean Doe" required>
              @error('name')
        ...
        <div class="form-group row mb-3">
          ...
          <div class="col-sm-9">
            <input type="email" id="email" value="{{ $user->email }}" name="email" class="form-control @error('email') is-invalid @enderror" placeholder="Email" required>
              @error('email')
                <span class="invalid-feedback">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="form-group row mb-3">
          ...
          <div class="col-sm-9">
            <input type="password" value="{{ old('password') }}" name="password" class="form-control @error('password') is-invalid @enderror" id="password" placeholder="Mot de passe">
              @error('password')
                <span class="invalid-feedback">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="form-group row mb-4">
          ...
          <div class="col-sm-9">
            <input type="password" value="{{ old('password') }}" name="password_confirmation" class="form-control @error('password_confirmation') is-invalid @enderror" id="password_confirm" placeholder="Confirmer le mot de passe">
        ...
        <div class="form-group mb-4">
          ...
          <select name="role" id="role" class="form-control @error('role') is-invalid @enderror">
            ...
            @foreach ($roles as $role)
                <option {{ $user->hasRole($role) ? 'selected' : '' }} value="{{ $role->id }}">{{ $role->name }}</option>
            @endforeach
          </select>
            @error('role')
              <span class="invalid-feedback">
